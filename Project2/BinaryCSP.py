...
def consistent(assignment, csp, var, value):
	...
	for c in csp.binaryConstraints:
		if c.affects(var):
			other = c.otherVariable(var)
			if assignment.isAssigned(other):
				if not c.isSatisfied(assignment.assignedValues[other], value):
					return False
	return True
...
def minimumRemainingValuesHeuristic(assignment, csp):
	...
	for var in domains:
		...
		if not assignment.isAssigned(var):
			degree = 0
			for c in csp.binaryConstraints:
				if c.affects(var) and not assignment.isAssigned(c.otherVariable(var)):
					degree = degree + 1
			minRemain = len(domains[var])
			if minRemain < m:
				m = minRemain
				nextVar = var
			if minRemain == m and degree > minDegree:
				minDegree = degree
				nextVar = var
	return nextVar
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	for value in values:
		...
		for x in csp.binaryConstraints:
			if x.affects(var):
				for y in assignment.varDomains[x.otherVariable(var)]:
					if not x.isSatisfied(y, value):
						priority = priority + 1
		p.put((priority, value))
	answer = []
	while not p.empty():
		answer.append(p.get()[1])
	return answer
...
def forwardChecking(assignment, csp, var, value):
	...
	for x in csp.binaryConstraints:
		if x.affects(var):
			other = x.otherVariable(var)
			if not assignment.isAssigned(other):
				t = domains[other]
				if len(domains[other]) <= 1:
					return None
				for y in domains[other]:
					if not x.isSatisfied(y, value):

						inferences.add((other, y))

	for i in inferences:
		domains[i[0]].remove(i[1])
...
def recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod):
	...
	for val in orderValuesMethod(assignment, csp, var):
		if consistent(assignment, csp, var, val):
			...
			answer = recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod)
			if (answer is None and inferences is not None):
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	q = Queue.Queue()
	for x in csp.binaryConstraints:
		q.put((x.var1, x.var2, x))
		q.put((x.var2, x.var1, x))
		# if x.affects(var):
		# 	q.put((var, x.otherVariable(var), x))
	while q.qsize() != 0:
		h, t, y = q.get()

		inf = revise(assignment, csp, h, t, y)
		if inf == None:
			for var, val in list(inferences):
				domains[var].update(val)
			return None
		if len(inf) != 0:
			for o in csp.binaryConstraints:
				if o.affects(t):
					q.put((t, o.otherVariable(t), o))
		inferences = set(inferences.union(inf))
	return inferences
