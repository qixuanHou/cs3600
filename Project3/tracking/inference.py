...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        overallDist = util.Counter()
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                overallDist[newPos] = overallDist[newPos] + prob * self.beliefs[oldPos]
        overallDist.normalize()
        self.beliefs = overallDist
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.particles:
                ...
                l[p] += emissionModel[trueDistance]
            if l.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                for i in range(self.numParticles):
                    self.particles[i] = util.sample(l)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        while num <= self.numParticles:
            for pos in self.legalPositions:
                self.particles += [tuple(pos for ghost in range(self.numGhosts))]
                num += 1
                if num  >= self.numParticles:
                    break
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        overall = util.Counter()

        for g in range(self.numGhosts):
            while weights[g].totalCount() ==0:
                for p in self.particles:
                    trueDistance = util.manhattanDistance(p[g], pacmanPosition)
                    weights[g][p[g]] +=emissionModels[g][trueDistance]
                if noisyDistances[g] == None:
                    break
                if weights[g].totalCount() ==0:
                    self.initializeParticles()
        newParticles = []
        for i in range(self.numParticles):
            np = []
            for g in range(self.numGhosts):
                if noisyDistances[g] == None:
                    np.append(self.getJailPosition(g))
                else:
                    np.append(util.sample(weights[g]))
            newParticles.append(tuple(np))
