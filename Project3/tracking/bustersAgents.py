...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        import operator
        best = [max(distribution.iteritems(), key = operator.itemgetter(1))[0]
                        for distribution
                        in livingGhostPositionDistributions]
        bp = min(best, key = lambda pos: self.distancer.getDistance(pos, pacmanPosition))
        newP = {action : self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), bp) for action in legal}
        return min(newP.iteritems(), key = operator.itemgetter(1))[0]
